...
function startGame() {
  ...
  // ===========================
  const enemyDieSoundPath = "audio/alan.mp3"; // ruta de tu sonido
  ...
  function gameLoop() {
    ...
    enemies.forEach((enemy, i) => {
      ...
      if (enemy.x < cat.x + cat.width && enemy.x + enemy.width > cat.x &&
        ...
        if (lives <= 0) {
          alert("😿 Nooooooooooooooooo, gano la izquierda pipipipipo  o la derecha xd xd  pero peirde el peru ja. Puntos: " + score);
    ...
    if (score >= 6) {
    cancelAnimationFrame(gameLoopId); // detenemos el loop actual
    cargarSiguienteNivel(); // llamamos al cambio
    return;
    }

    gameLoopId = requestAnimationFrame(gameLoop); // 🔥 guardamos el id
  }
